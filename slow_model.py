...
class ResidualBlock(torch.nn.Module):
    ...
    def __init__(self,
        ...
        super(ResidualBlock, self).__init__() # number of channels after a block is always *4 what it was when it entered 
        #NOTE: DO WE NEED ResidualBlock in the superinit?
        ...
        self.conv2 = torch.nn.Conv3d(
            ...
            kernel_size=(1,3,3), 
            stride=stride,
            padding=(0,1,1), # should not padding be 1?
            bias=True
        )
        ...
        #self.identity_downsample = identity_downsample # a conv layer for the identity mapping so that its of the same shape later on in th layers
         # we might need it later


    def forward(self, x):

        # outputs from dif layers
...
class SlowNet(torch.nn.Module):
    ...
    def __init__(self, 
        ...
        self.conv1_slow = torch.nn.Conv3d(
            ...
            out_channels=64, 
            kernel_size=(1,7,7), 
            stride=(1,3,3) # MADE changes here, stride from 222 to 133
        )
        ...
        self.linear_slow = torch.nn.Linear(2048, number_of_classes) # NOTE: had to remove sequential, as it was throwing errors
    ...
    def _make_residual_layer(self,
                             ...
                             intermediate_channels,
                             stride=1
                             ): # as per paper the first layer in block stride 1, and then 2
        ...
        # print(f'in_channels - {self.in_channels}, intermediate_channels - {intermediate_channels}, stride: {stride}')
        self.in_channels = intermediate_channels*4

        for i in range(number_of_blocks -1):
            layers.append(ResidualBlock(self.in_channels, intermediate_channels, block_ver))
            # print(f'block_{i}: in_channels - {self.in_channels}, intermediate_channels - {intermediate_channels}, stride: {stride}')
